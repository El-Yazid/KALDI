...
echo
echo "===== TRI2 (second triphone pass) TRAINING [delta+delta-deltas] ====="
echo
...
echo
echo "===== TRI2b (second triphone pass) TRAINING+DECODING [LDA+MLLT] ====="
echo
steps/align_si.sh --nj $nj  --cmd "$train_cmd" data/train data/lang exp/tri2 exp/tri2_ali || exit 1;
echo
echo "===== TRI3a (third triphone pass) TRAINING LDA+MLLT]====="
echo
...
steps/decode_fmllr.sh --cmd "$decode_cmd" --nj $nj --config conf/decode.config exp/tri4a/graph data/test exp/tri4a/decode
steps/align_fmllr.sh  --cmd "$train_cmd" --nj $nj  data/train data/lang exp/tri4a exp/tri4a_ali
echo
echo "=====Building a larger SAT system====="
echo
...
steps/decode_fmllr.sh --cmd "$decode_cmd" --nj $nj --config conf/decode.config exp/tri5a/graph data/test exp/tri5a/decode || exit 1;
...
for x in exp/*/decode; do [ -d $x ] && grep WER $x/cer_* | utils/best_wer.sh; done 2>a.txt
for x in exp/*/*/decode; do [ -d $x ] && grep WER $x/cer_* | utils/best_wer.sh; done 2>b.txt
